/*
  ...
  1. Багажник (D6). В разделе «Входы» сказано «управление — низкий
     уровень», а в Условии 5.1 — «Если Pin 6 = HIGH». Принят HIGH, по
     букве Условия 5.1.
     ВАЖНО ПРО ПРОВОДКУ: вход с INPUT_PULLUP сам по себе висит на HIGH,
     поэтому «спокойное» состояние здесь — это LOW. Датчик багажника
     должен ЗАМЫКАТЬ D6 на массу, пока багажник ЗАКРЫТ, и отпускать при
     открытии. Если провод отвалится или датчик разомкнётся, лента
     начнёт мигать жёлтым — это нормально для такой схемы, так и
     задумано. Обратно на общий для всех входов активный LOW:
     поставьте TRUNK_ACTIVE_LEVEL LOW.
...
#define TRUNK_ACTIVE_LEVEL HIGH    // усл. 5.1: багажник срабатывает по HIGH,
                                   // см. расхождение №1 в шапке файла
